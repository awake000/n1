...
#runfrom=$(lsblk | grep '/var/log.hdd' | grep -oE '(mmcblk[0-9]|sda[0-9])')
runfrom=$(findmnt -n -o SOURCE / | grep -oE '(mmcblk[0-9]|[h|s|v]d[a-z][0-9])')
if [ "$runfrom" = "" ]
then
...
echo "Unmount disks mounted by trim"
umount /vol*/*

fstype=$(lsblk -no FSTYPE /dev/$runfrom)
case "$fstype" in
btrfs|\
ext4)
	;;
*)
	echo "UNSUPPORTED FILESYSTEM TYPE $fstype ON ROOT PARTITION /dev/$runfrom"
	exit 1
	;;
esac
...
disk_size="$(blockdev --getsize64 "${DEV_EMMC}")"
if [ -n "$disk_size" ] && [ "$((disk_size / 1024 / 1024 / 1024))" -gt "32" ]; then
    rootfs_size="28G"
else
    rootfs_size="100%"
fi
...
parted -s "${DEV_EMMC}" mkpart primary "$fstype" 1213M "$rootfs_size"
...
sed -e "/external_rootpart_uuid/d" \
 -i "$DIR_INSTALL/fnEnv.txt"
echo "done."

rm $DIR_INSTALL/aml_autoscript*
...
case "$fstype" in
btrfs)
	mkfs.btrfs -d single -f -L ROOT_EMMC $PART_ROOT
	;;
ext4)
...
case "$fstype" in
btrfs)
	mount -o rw,compress=zstd $PART_ROOT $DIR_INSTALL
	;;
ext4)
...
echo -n "Edit fstab..."
sed -e "s,UUID=.*\t/boot,UUID=$(blkid -s UUID -o value "${PART_BOOT}")\t/boot," \
 -e "s,UUID=.*\t/\t,UUID=$(blkid -s UUID -o value "${PART_ROOT}")\t/\t," \
